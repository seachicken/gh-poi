...
func (conn *Connection) RunShellCommand(ctx context.Context, command string, arg string) (string, error) {
	args := []string{"-c", command + ` "$1"`, "--", arg}
	return conn.run(ctx, "sh", args, None)
}
